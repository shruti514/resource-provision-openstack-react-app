    ...
    if (user) {
        if ("Testing" === user.department) {
            var testingClient = new Nova({
                ...
                var glanceClientTestingDept = new Glance({
                    url: 'http://localhost:5000/v2.0/',
                    debug: true
                }).authenticate({
                    username: 'TestingAccount',
                    ...
                    testingClient.servers.all({
                        ...
                        var toReturn = [];
                        if (servers) {
                            glanceClientTestingDept.images.all({async: false}, function (err, images) {
        ...
        if ("Development" === user.department) {
            var developmentClient = new Nova({
                ...
                var glanceClientForDevelTeam = new Glance({
                    url: 'http://localhost:5000/v2.0/',
                    debug: true
                }).authenticate({
                    username: 'DeveloperAccount',
                    ...
                    developmentClient.servers.all({
                        ...
                        var toReturn = [];
                        if (servers) {
                            glanceClientForDevelTeam.images.all({async: false}, function (err, images) {
                                //var toReturn = image.name;
